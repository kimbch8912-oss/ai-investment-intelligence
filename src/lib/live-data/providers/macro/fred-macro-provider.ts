...
export class FredMacroProvider {
  async getSnapshot(analysisTime: string): Promise<FredMacroSnapshot> {
    ...
    const fetchSeries = async (seriesId: FredSeriesId) => { try { const end = requestEndDate(seriesId, analysisTime, retrievedAt); const vintage: Record<string, string> = daily.has(seriesId) ? { realtime_start: end, realtime_end: end } : { realtime_end: utcDate(analysisTime) }; const query = new URLSearchParams({ series_id: seriesId, api_key: key, file_type: 'json', observation_start: '2020-01-01', observation_end: end, ...vintage }); const response = await fetch(`${base()}?${query}`, { cache: 'force-cache', next: { revalidate: ttl(seriesId), tags: [`fred:${seriesId}:${end}`] } }); const payload = await response.json() as { observations?: Array<{ date?: string; value?: string; realtime_end?: string }> }; if (!response.ok || !Array.isArray(payload.observations)) throw Error(seriesId); return payload.observations.map((row) => ({ seriesId, observationDate: String(row.date), value: String(row.value), vintageAt: `${row.realtime_end ?? row.date}T23:59:59.999Z`, retrievedAt, frequency: frequency[seriesId], source: 'FRED' as const, status: 'READY' as const })).filter((row) => validObservation(row, analysisTime)); } catch { throw Error(seriesId); } };
    const settled = await Promise.allSettled(fredSeries.map(fetchSeries)); const observations: FredObservation[] = []; const errors: string[] = []; settled.forEach((result, index) => { if (result.status === 'fulfilled') { observations.push(...result.value); if (!result.value.length) errors.push(`${fredSeries[index]}_UNAVAILABLE`); } else errors.push(fredSeries[index]); }); return { observations, errors, retrievedAt };
  }
}
