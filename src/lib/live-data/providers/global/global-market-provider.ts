...
export class TwelveDataGlobalMarketProvider implements MarketDataProvider {
  ...
  async getMarketData(request: MarketDataRequest): Promise<SourceEnvelope<MarketDataSnapshot>> {
    ...
    try {
      const response = await fetchWithTimeout(`${baseUrl()}/time_series?${query}`, { headers: { Authorization: `apikey ${apiKey}` }, cache: 'force-cache', next: { revalidate: 1_800, tags: [`market:${request.asset.symbol}`] } } as RequestInit, 8_000);
    ...
    } catch (error) { return fail(isProviderTimeout(error) ? 'PROVIDER_TIMEOUT' : error instanceof Error && error.message === 'LOOK_AHEAD_BLOCKED' ? 'LOOK_AHEAD_BLOCKED' : 'PROVIDER_NETWORK_ERROR', 'Market provider request or validation failed.'); }
  }
}
